...
ALTER TABLE quiz_questions ADD CONSTRAINT check_quiz_difficulty
...
ALTER TABLE groups ADD CONSTRAINT check_privacy_setting
    CHECK (privacy_setting IN ('PUBLIC', 'PRIVATE', 'INVITATION_ONLY'));

ALTER TABLE quests ADD CONSTRAINT check_quest_type
    CHECK (type IN ('CHALLENGE', 'ACTIVITY_PARTNER', 'LEARNING', 'CONTEST'));

ALTER TABLE quests ADD CONSTRAINT check_quest_visibility
    CHECK (visibility IN ('PUBLIC', 'PRIVATE', 'GROUP_ONLY'));

ALTER TABLE quests ADD CONSTRAINT check_quest_status
...
ALTER TABLE tasks ADD CONSTRAINT check_verification_method
    CHECK (verification_method IN ('MANUAL', 'FITNESS_API', 'PHOTO', 'QUIZ', 'LOCATION', 'ACTIVITY'));

ALTER TABLE task_completions ADD CONSTRAINT check_completion_status
    CHECK (status IN ('SUBMITTED', 'VERIFIED', 'REJECTED'));

ALTER TABLE rewards ADD CONSTRAINT check_reward_type
    CHECK (type IN ('MONETARY', 'POINTS', 'BADGE', 'CUSTOM'));

ALTER TABLE rewards ADD CONSTRAINT check_reward_source
    CHECK (reward_source IN ('INDIVIDUAL', 'GROUP', 'SYSTEM'));

ALTER TABLE rewards ADD CONSTRAINT check_currency
    CHECK (currency IN ('USD', 'EUR', 'GBP', 'CAD', 'AUD', 'POINTS'));

ALTER TABLE user_quests ADD CONSTRAINT check_user_quest_status
    CHECK (status IN ('ACTIVE', 'COMPLETED', 'ABANDONED'));

ALTER TABLE group_users ADD CONSTRAINT check_user_role
    CHECK (role IN ('ADMIN', 'MEMBER', 'MODERATOR'));

ALTER TABLE user_connections ADD CONSTRAINT check_connection_status
    CHECK (status IN ('PENDING', 'ACCEPTED', 'REJECTED', 'BLOCKED'));

ALTER TABLE user_activity_logs ADD CONSTRAINT check_activity_type
    CHECK (activity_type IN ('TASK_COMPLETION', 'JOIN_GROUP', 'COMPLETE_QUEST', 'REWARD_EARNED', 'CONNECTION_MADE', 'CHALLENGE_JOINED', 'CHALLENGE_COMPLETED'));
...
ALTER TABLE challenge_progress ADD CONSTRAINT check_progress_status
