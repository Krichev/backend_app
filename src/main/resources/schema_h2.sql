...
-- Add media_url column to quiz_questions table
ALTER TABLE quiz_questions ADD COLUMN IF NOT EXISTS media_url VARCHAR(500);
ALTER TABLE quiz_questions ADD COLUMN IF NOT EXISTS has_media BOOLEAN DEFAULT FALSE;
ALTER TABLE quiz_questions ADD COLUMN IF NOT EXISTS media_type VARCHAR(20); -- IMAGE, VIDEO, AUDIO

-- Create dedicated media table for all file types
CREATE TABLE IF NOT EXISTS media_files (
                                           id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                                           filename VARCHAR(255) NOT NULL,
                                           original_filename VARCHAR(255) NOT NULL,
                                           storage_key UUID DEFAULT RANDOM_UUID() NOT NULL,
                                           content_hash VARCHAR(64),
                                           file_path VARCHAR(500) NOT NULL,
                                           file_size BIGINT,
                                           mime_type VARCHAR(100),
                                           media_category VARCHAR(20), -- IMAGE, VIDEO, AUDIO
    ...
    -- Video/Audio specific fields
                                           ...
                                           bitrate INTEGER,
                                           resolution VARCHAR(20), -- For videos: "1920x1080"
    ...
    -- Thumbnail for videos
                                           ...
                                           uploaded_by BIGINT,
                                           media_type VARCHAR(50), -- AVATAR, QUIZ_QUESTION, VERIFICATION, CHALLENGE
...
CREATE INDEX IF NOT EXISTS idx_media_entity ON media_files (entity_id, media_type);
CREATE INDEX IF NOT EXISTS idx_media_category ON media_files (media_category);
CREATE INDEX IF NOT EXISTS idx_media_processing ON media_files (processing_status);
...
-- Challenge-Question Assignments (many-to-many junction)
CREATE TABLE IF NOT EXISTS challenge_question_assignments (
    id              BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    challenge_id    BIGINT NOT NULL,
    question_id     BIGINT NOT NULL,
    assignment_type VARCHAR(30) NOT NULL DEFAULT 'SELECTED',
    ...
    CONSTRAINT uq_challenge_question UNIQUE (challenge_id, question_id)
);
...
CREATE INDEX IF NOT EXISTS idx_cqa_challenge_id ON challenge_question_assignments(challenge_id);
CREATE INDEX IF NOT EXISTS idx_cqa_question_id ON challenge_question_assignments(question_id);
